...
from models import get_model, load_model, unittest_model, build_optimizer, test
from gnns import GNNRemesher
from datasets.load_dataset import load_data
from pytorch_net.util import Attr_Dict, Batch, filter_filename, pload, pdump, Printer, get_time, init_args, update_args, clip_grad, set_seed, update_dict, filter_kwargs, plot_vectors, plot_matrices, make_dir, get_pdict, to_np_array, record_data, make_dir, Early_Stopping, str2bool, get_filename_short, print_banner, get_num_params, ddeepcopy as deepcopy, write_to_config
from utils import EXP_PATH, MeshBatch, OneDMeshBatch
from utils import p, update_legacy_default_hyperparam, get_grad_norm, loss_op_core, get_model_dict, get_elements, is_diagnose, get_keys_values, loss_op, to_tuple_shape, parse_multi_step, get_device, seed_everything
...
if args.dataset.startswith("m"):
    ...
    args_test8.multi_step = "1^30" 
    args_test8.pred_steps = 30
    args_test8.is_train = False 
...
elif args.rl_is_finetune_evolution and args.dataset.startswith("tetra"):
    args_testfine = deepcopy(args)
    args_testfine.use_fineres_data = True
    (dataset_train_val_fine,_), (train_loader_fine, _, _) = load_data(args_testfine)
    p.print(f"Minibatches for trainfine: {len(train_loader_fine)}")
...
if args.evo_transfer: 
    ...
    model = load_model(data_record_load["model_dict"][-1], device)
else :
...
if args.algo.startswith("rlgnnremesher") or args.algo.startswith("srlgnnremesher"): 
    ...
    args_load = init_args(update_legacy_default_hyperparam(data_record_load["args"]))
    args_load.multi_step = args.multi_step
    evolution_model = load_model(data_record_load["best_model_dict"], device)
    if args.fix_alt_evolution_model:
        evolution_model_alt = load_model(data_record_load["best_model_dict"], device)
...
if args.load_hash!="None":
    data_record_load = find_hash_and_load(all_hash = [args.load_hash], dirname = args.load_hash_filename)
...
if data.__class__.__name__ == "HeteroGraph":
    collate_fn = deepsnap_Batch.collate()
elif args.dataset.startswith("arcsimmesh") or  args.dataset.startswith("tetramesh"):
    collate_fn = MeshBatch(is_absorb_batch=True, is_collate_tuple=True).collate()
else:
    collate_fn = Batch(is_absorb_batch=True, is_collate_tuple=True).collate()
...
if args.wandb:
    ...
    wandb.init(project=args.wandb_project_name, entity="team-fengbo", name="_".join(["beta_",args.reward_beta]+filename.split("_")[-2:])[:-2] + f'_ntrain_{args.n_train}{"_" + args.id if args.id != "0" else ""}', config={"name": args.exp_id})
...
while epoch < args.epochs:
    ...
    if args.rl_is_finetune_evolution and args.dataset.startswith("tetra"):
        train_loader_fine_iterator = iter(train_loader_fine)

    for j, data in enumerate(train_loader):
        if args.rl_is_finetune_evolution and (args.dataset.startswith("a") or args.dataset.startswith("tetra")):
            try:
    ...
    for jj, (key, value) in enumerate(train_info.items()):
        if jj % 2 == 0:
            print(f"{key}_train:   \t{np.mean(value):.6f}", end="\n")
        else:
            print(f"\t{key}_train:   \t{np.mean(value):.6f}  ")
    ...
    data_record["last_model_dict"] = get_model_dictrainsett(model)
