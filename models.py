...
sys.path.append(os.path.join(os.path.dirname("__file__"), '..', '..'))
from gnns import GNNRemesher, Value_Model_Summation, Value_Model, GNNRemesherPolicy, GNNPolicySizing, GNNPolicyAgent, get_reward_batch, get_data_dropout, GNNPolicyAgent_Sampling
from datasets.arcsimmesh_dataset import get_2d_data_pred
from datasets.tetramesh_dataset import get_3d_data_pred
from datasets.mppde1d_dataset import get_data_pred, update_edge_attr_1d
from utils_model import get_conv_func, get_conv_trans_func, get_Hessian_penalty
from pytorch_net.util import get_repeat_interleave, Printer, forward_Runge_Kutta, tuple_add, tuple_mul, clip_grad, Batch, make_dir, to_np_array, record_data, make_dir, ddeepcopy as deepcopy, filter_filename, Early_Stopping, str2bool, get_filename_short, print_banner, plot_matrices, to_string, init_args, get_poly_basis_tensor, get_num_params, get_pdict
from utils import PDE_PATH, EXP_PATH
from utils import sample_reward_beta, copy_data, requires_grad, endow_grads, process_data_for_CNN, get_regularization, get_batch_size
from utils import detach_data, get_model_dict, loss_op_core, MLP, MLP_Coupling, get_keys_values, flatten, get_activation, to_cpu, to_tuple_shape, parse_multi_step, parse_act_name, parse_reg_type, loss_op, get_normalization, add_noise, get_neg_loss, get_pos_dims_dict
from utils import p, seed_everything, is_diagnose, get_precision_floor, parse_string_idx_to_list, parse_loss_type, get_loss_ar, get_max_pool, get_data_next_step, expand_same_shape, add_data_noise
from pytorch_net.util import Attr_Dict, set_seed, pdump, pload, get_time, check_same_model_dict, Zip, Interp1d_torch
        ...
        **kwargs
        ...
        if not len(dict(to_tuple_shape(data.original_shape))["n0"]) == 0:
            if args.rl_data_dropout.startswith("node"):
                ...
                data = get_data_dropout(data, dropout_mode=args.rl_data_dropout, sample_idx=sample_idx,output_size=25)
    ...
    def get_value_loss(self, data, args, state_preds, rewards,sampled_reward=None, value_target_mode="value-lambda"):
        ...
        for k in range(self.horizon):
            ...
            if len(dict(to_tuple_shape(data.original_shape))["n0"]) == 0:
                ...
                if args.stress:
                    stress_list = [state_pred["history_stress"][-1] for state_pred in state_preds]            

                data_pred = get_3d_data_pred(
                    state_preds=x_list,
                    state_preds_stress=stress_list,
                    step=k,
                    data=data,
...
def generate_tetramesh(vers,faces,tarvers):
    mesh = dolfin.Mesh()
    editor = dolfin.MeshEditor()
    editor.open(mesh, 'tetrahedron', 3, 3)
    editor.init_vertices(vers.shape[0])
    for i in range(vers.shape[0]):
        editor.add_vertex(i, vers[i,:3].cpu().numpy())
    editor.init_cells(faces.shape[0])
    for f in range(faces.shape[0]):
        editor.add_cell(f, faces[f].cpu().numpy())
    editor.close()
...
def generate_baryweight_core_tetramesh(vers,faces,tarvers,mesh,bvh_tree):
    faces = []
    weights = []
    # pdb.set_trace()
    for query in tarvers:
...
def generate_baryweight_tetramesh(vers, faces, tarvers):
    mesh, bvh_tree = generate_tetramesh(vers,faces,tarvers)
    faces, weights, mesh = generate_baryweight_core_tetramesh(vers,faces,tarvers,mesh,bvh_tree)
    # p.print("99995", precision="millisecond", is_silent=False, avg_window=1)
    # pdb.set_trace()
    return faces, weights, mesh
...
def generate_barycentric_interpolated_data_tetramesh(vers, faces, outvec,outvec_stress, tarvers):
    faces, weights, mesh = generate_baryweight_tetramesh(vers, faces, tarvers)
    indices = mesh.cells()[faces].astype('int64')
    fweights = torch.tensor(np.array(weights), device=outvec.device, dtype=torch.float32)
    return torch.matmul(fweights, outvec[indices,:]).diagonal().T, torch.matmul(fweights, outvec_stress[indices,:]).diagonal().T# [:,:self.output_size].reshape(-1,1,self.output_size)

def get_loss_3d_refined_tetramesh(data_pred, data_gt, current_step,opt_evl=False,stress = False,original_mesh = False):
    if not opt_evl:
        # data_pred = data_pred.clone()
        ybatch = ((data_gt.reind_yfeatures_finegrain["n0"][current_step][:,0] - data_gt.yfeatures_finegrain["n0"][current_step][:,0])/2)
        vers = (data_pred.history[-1] + 2*data_pred.batch.repeat((data_pred.history[-1].shape[1], 1)).T)
        faces = data_pred.xfaces.T
        tarvers = data_gt.reind_yfeatures_finegrain["n0"][current_step][:,:3].clone().detach().cpu().numpy()
        stress_presd = data_pred.stress[:,:1]
        # p.print("9991", precision="millisecond", is_silent=False, avg_window=1)
        interpolate_out,interpolate_out_stress = generate_barycentric_interpolated_data_tetramesh(vers.clone().detach(), faces, vers, stress_presd,tarvers)
        ...
        mse = (data_gt.yfeatures_finegrain["n0"][current_step][:,3:] - gt).square()
        
        loss = batch_mse(torch.sqrt(mse.sum(dim=1)), ybatch)[:,None]
        loss_mse = batch_mse((mse.sum(dim=1)), ybatch, mean=False)[:,None]

        if stress : 
            gt_stress = interpolate_out_stress
            mse_stress = (data_gt.y_back_stress_finegrain["n0"][current_step] - gt_stress).square()
            loss = loss  + 0.1 *  batch_mse(torch.sqrt(mse_stress.sum(dim=1)), ybatch)[:,None]
            loss_mse = loss_mse  + 0.001 * batch_mse((mse_stress.sum(dim=1)), ybatch, mean=False)[:,None]
            
        return loss,loss_mse, None
    else:
        ybatch = data_pred.batch
        vers = data_gt.reind_yfeatures_finegrain["n0"][current_step] 
        faces = torch.tensor(data_gt.yface_list_finegrain["n0"][current_step])    
        tarvers = data_pred.history[-1][:,:3] + 2*data_pred.batch.repeat((data_pred.history[-1].shape[1], 1)).T[:,:3]
        if isinstance(tarvers, torch.Tensor):
            tarvers = tarvers.detach().cpu().numpy()
        stress_presd = data_gt.y_back_stress_finegrain["n0"][current_step][:,:1]
        # Interpolation and compute kinematics
        interp_gtar, interp_gtar_stress = generate_barycentric_interpolated_data_tetramesh(vers, faces, vers,stress_presd,tarvers)
        ...
        if stress: 
            interp_stress = interp_gtar_stress 
            mse_stress = (data_pred.stress[:,0:1]-interp_stress).square()
            loss = loss + 0.1 * batch_mse(torch.sqrt(mse_stress.sum(dim=1)), ybatch)[:,None]
            loss_mse = loss_mse + 0.001* batch_mse((mse_stress.sum(dim=1)), ybatch, mean=False)[:,None]
        # pdb.set_trace()
        return loss,loss_mse, interp_gtar_coords
...
def full_forward(
    ...
    if len(dict(to_tuple_shape(data.original_shape))["n0"]) == 0:
        ...
        state_preds = Attr_Dict({
            ...
            "y_back_stress": data["y_back_stress"]["n0"],
            "y_back_stress_finegrain": data["y_back_stress_finegrain"]["n0"],
            "y_tar": data["y_tar"]["n0"],
            "y_tar_stress": data["y_tar_stress"]["n0"],
            "yface_list": data["yface_list"]["n0"],
            "yface_list_finegrain": data["yface_list_finegrain"]["n0"],
            "history": data["history"]["n0"],
            "history_stress": data["history_stress"]["n0"],
            "yfeatures": data["yfeatures"]["n0"],
            "yfeatures_finegrain": data["yfeatures_finegrain"]["n0"],
            "yfeatures_stress": data["yfeatures_stress"]["n0"],
            "yfeatures_stress_finegrain": data["yfeatures_stress_finegrain"]["n0"],
            "node_dim": data["node_dim"]["n0"],
            "xface_list": data["xface_list"]["n0"],
            "reind_yfeatures": data["reind_yfeatures"]["n0"],
            "reind_yfeatures_finegrain": data["reind_yfeatures_finegrain"]["n0"],
            "reind_yfeatures_stress": data["reind_yfeatures_stress"]["n0"],
            "reind_yfeatures_stress_finegrain": data["reind_yfeatures_stress_finegrain"]["n0"],
            "batch": data.batch["n0"],
    ...
    if opt_evl and args.rl_finetune_evalution_mode in ["policy:fine","policy:alt:fine"]:
        if args.dataset.startswith("m"):
            ...
            state_preds_fine = Attr_Dict({
                ...
                "y_back_stress": data_fine["y_back_stress"]["n0"],
                "y_back_stress_finegrain": data_fine["y_back_stress_finegrain"]["n0"],
                "y_tar": data_fine["y_tar"]["n0"],
                "y_tar_stress": data_fine["y_tar_stress"]["n0"],
                "yface_list": data_fine["yface_list"]["n0"],
                "yface_list_finegrain": data_fine["yface_list_finegrain"]["n0"],
                "history": data_fine["history"]["n0"],
                "history_stress": data_fine["history_stress"]["n0"],
                "yfeatures": data_fine["yfeatures"]["n0"],
                "yfeatures_stress": data_fine["yfeatures_stress"]["n0"],
                "yfeatures_finegrain": data_fine["yfeatures_finegrain"]["n0"],
                "yfeatures_stress_finegrain": data_fine["yfeatures_stress_finegrain"]["n0"],
                "node_dim": data_fine["node_dim"]["n0"],
                "xface_list": data_fine["xface_list"]["n0"],
                "reind_yfeatures": data_fine["reind_yfeatures"]["n0"],
                "reind_yfeatures_finegrain": data_fine["reind_yfeatures_finegrain"]["n0"],
                "reind_yfeatures_stress": data_fine["reind_yfeatures_stress"]["n0"],
                "reind_yfeatures_stress_finegrain": data_fine["reind_yfeatures_stress_finegrain"]["n0"],
    ...
    if args.dataset.startswith("mppde"):
        sep_length = 10
    elif args.dataset.startswith("arc"):
        sep_length = 500
    elif args.dataset.startswith("tetra"):
        sep_length = 128
    ...
    for k in range(horizon):  # k=0,1,...H-1
        ...
        if (opt_evl and args.evl_stop_gradient and ( args.dataset.startswith("a") or args.dataset.startswith("tetra"))) or args.stop_all_gradient:
        ...
        if len(dict(to_tuple_shape(data.original_shape))["n0"]) == 0:
            ...
            state_preds = Attr_Dict({
                ...
                "y_back": data.y_back,
                "y_back_stress": data.y_back_stress,
                "y_tar": data.y_tar,
                "y_tar_stress": data.y_tar_stress,
                "yface_list": data.yface_list,
                "yface_list_finegrain": data.yface_list_finegrain,
                "history": [elm.clone().detach() for elm in data.history],
                "history_stress": [elm.clone().detach() for elm in data.history_stress],
                "yfeatures": data.yfeatures,
                "yfeatures_stress": data.yfeatures_stress,
                "yfeatures_finegrain": data.yfeatures_finegrain,
                "yfeatures_stress_finegrain": data.yfeatures_stress_finegrain,
                "node_dim": data.node_dim,
                "xface_list": data.xface_list,
                "reind_yfeatures": data.reind_yfeatures,
                "reind_yfeatures_finegrain": data.reind_yfeatures_finegrain,
                "reind_yfeatures_stress": data.reind_yfeatures_stress,
                "reind_yfeatures_stress_finegrain": data.reind_yfeatures_stress_finegrain,
                "batch": data.batch,
        ...
        if heuristic:
            ...
            state_preds_heuristic_gt_evl = Attr_Dict({
                ...
                "x": velocity.clone().detach(),
                "stress":data_heuristic_gt_evl.stress,
                ...
                "y_back_stress": data_heuristic_gt_evl.y_back_stress,
                ...
                "reind_yfeatures_stress": data_heuristic_gt_evl.reind_yfeatures_stress,
        ...
        if not (mode=="test_remeshing"):
            ...
            if len(dict(to_tuple_shape(data_alt.original_shape))["n0"]) == 0:
                ...
                state_preds_alt = Attr_Dict({
                    ...
                    "y_back_stress": data_alt.y_back_stress,
                    "y_back_stress_finegrain": data_alt.y_back_stress_finegrain,
                    "y_tar": data_alt.y_tar,
                    "y_tar_stress": data_alt.y_tar_stress,
                    "yface_list": data_alt.yface_list,
                    "yface_list_finegrain": data_alt.yface_list_finegrain,
                    "history": [elm.clone().detach() for elm in data_alt.history],
                    "history_stress": [elm.clone().detach() for elm in data_alt.history_stress],
                    "yfeatures": data_alt.yfeatures,
                    "yfeatures_finegrain": data_alt.yfeatures_finegrain,
                    "yfeatures_stress": data_alt.yfeatures_stress,
                    "yfeatures_stress_finegrain": data_alt.yfeatures_stress_finegrain,
                    "node_dim": data_alt.node_dim,
                    "xface_list": data_alt.xface_list,
                    "reind_yfeatures": data_alt.reind_yfeatures,
                    "reind_yfeatures_finegrain": data_alt.reind_yfeatures_finegrain,
                    "reind_yfeatures_stress": data_alt.reind_yfeatures_stress,
                    "reind_yfeatures_stress_finegrain": data_alt.reind_yfeatures_stress_finegrain,
        ...
        if mode.startswith("test_gt_remesh"):
            ...
            state_preds_alt_gt_evl = Attr_Dict({
                ...
                "y_back_stress": data_alt_gt_evl.y_back_stress,
                ...
                "reind_yfeatures_stress": data_alt_gt_evl.reind_yfeatures_stress,
            ...
            state_preds_alt_gt_mesh_gt_evl = Attr_Dict({
                ...
                "x": velocity.clone().detach(),
                "stress":data_alt_gt_mesh_gt_evl.stress,
                ...
                "y_back_stress": data_alt_gt_mesh_gt_evl.y_back_stress,
                ...
                "reind_yfeatures_stress": data_alt_gt_mesh_gt_evl.reind_yfeatures_stress,
            ...
            state_preds_alt_008_gt_evl = Attr_Dict({
                ...
                "x": velocity.clone().detach(),
                "stress":data_alt_008_gt_evl.stress,
                ...
                "y_back_stress": data_alt_008_gt_evl.y_back_stress,
                ...
                "reind_yfeatures_stress": data_alt_008_gt_evl.reind_yfeatures_stress,
        ...
        if opt_evl and args.rl_finetune_evalution_mode in ["policy:fine","policy:alt:fine"]:    
            if len(dict(to_tuple_shape(data.original_shape))["n0"]) == 0:
                ...
                state_preds_fine = Attr_Dict({
                    ...
                    "y_back_stress": data_fine.y_back_stress,
                    "y_tar": data_fine.y_tar,
                    "y_tar_stress": data_fine.y_tar_stress,
                    "yface_list": data_fine.yface_list,
                    "yface_list_finegrain": data_fine.yface_list_finegrain,
                    "history": [elm.clone().detach() for elm in data_fine.history],
                    "history_stress": [elm.clone().detach() for elm in data_fine.history_stress],
                    "yfeatures": data_fine.yfeatures,
                    "yfeatures_finegrain": data_fine.yfeatures_finegrain,
                    "yfeatures_stress": data_fine.yfeatures_stress,
                    "yfeatures_stress_finegrain": data_fine.yfeatures_stress_finegrain,
                    "node_dim": data_fine.node_dim,
                    "xface_list": data_fine.xface_list,
                    "reind_yfeatures": data_fine.reind_yfeatures,
                    "reind_yfeatures_finegrain": data_fine.reind_yfeatures_finegrain,
                    "reind_yfeatures_stress": data_fine.reind_yfeatures_stress,
                    "reind_yfeatures_stress_finegrain": data_fine.reind_yfeatures_stress_finegrain,
        ...
        if args.reward_src == "env":
            ...
            if len(dict(to_tuple_shape(data.original_shape))["n0"]) == 0:
                # pdb.set_trace()
                loss, loss_mse, data_gt = get_loss_3d_refined_tetramesh(data, data_finegrain, k, opt_evl=opt_evl,stress=stress)
                if mode.startswith("test_gt_remesh"):
                    loss_alt, loss_alt_mse, _ = get_loss_3d_refined_tetramesh(data_alt, data_finegrain, k, opt_evl=opt_evl)
                    loss_alt_gt_evl, loss_alt_gt_evl_mse, _ = get_loss_3d_refined_tetramesh(data_alt_gt_evl, data_finegrain, k, opt_evl=opt_evl)
                    ...
                    loss_alt_gt_mesh_gt_evl, loss_alt_gt_mesh_gt_evl_mse, _ = get_loss_3d_refined_tetramesh(data_alt_gt_mesh_gt_evl, data_finegrain, k, opt_evl=opt_evl)
                    ...
                    loss_alt_008_gt_evl, loss_alt_008_gt_evl_mse, _ = get_loss_3d_refined_tetramesh(data_alt_008_gt_evl, data_finegrain, k, opt_evl=opt_evl)
                    ...
                    if heuristic:
                        loss_heuristic_gt_evl, loss_heuristic_gt_evl_mse, _ = get_loss_3d_refined_tetramesh(data_heuristic_gt_evl, data_finegrain, k, opt_evl=opt_evl)
                    ...
                    loss_alt, loss_alt_mse, data_alt_gt = get_loss_3d_refined_tetramesh(data_alt, data_finegrain, k, opt_evl=opt_evl, original_mesh=True,stress=stress)
            ...
            if opt_evl and (args.rl_finetune_evalution_mode in ["policy:fine","policy:alt:fine"]):  
                if len(dict(to_tuple_shape(data.original_shape))["n0"]) == 0:
                    # pdb.set_trace()
                    loss_fine, loss_fine_mse, data_fine_gt = get_loss_3d_refined_tetramesh(data_fine, data_finegrain_fine, k, opt_evl=opt_evl)
...
def get_model(
    ...
    if args.algo.startswith("fno"):
        ...
        diffMLP = True if args.algo.endswith("diff") else False
        if len(dict(data_eg.original_shape)["n0"]) == 1:
            model = GNNRemesher(
                input_size=input_size["n0"]+ 1 + (1 if args.use_pos else 0) + (3 if args.dataset.startswith("mppde1dh") else 0),  # +1 for x_bdd (boundary nodes), +1 for use_pos, +3 for parameters
                output_size=output_size["n0"] * args.temporal_bundle_steps,
                edge_dim=1 if args.dataset.startswith("mppde1de") else 2*(input_size["n0"]+1),
                sizing_field_dim=0,
            ...
            model = GNNRemesherPolicy(
                input_size=(2*input_size[key] + 9 +data_eg.onehot_list["n0"][0].shape[-1] if "onehot_list" in data_eg else input_size["n0"]), 
                output_size=output_size["n0"] * args.temporal_bundle_steps,
        ...
        if len(dict(data_eg.original_shape)["n0"]) == 1:
            ...
            sampler_model = GNNPolicyAgent_Sampling(
                ...
                edge_dim=1 if args.dataset.startswith("mppde1de") else (25+1)*2,
                nmax=torch.tensor(50000),
                latent_dim=args.latent_size,
                edge_attr=args.edge_attr,
                layer_norm=args.layer_norm,
    ...
    with torch.no_grad():
        ...
        if "pred" in test_cases and model_dict["type"] not in ["Actor_Critic"]:
            ...
            if is_mesh:
                ...
                pred["n0"] = pred["n0"][0]
                pred2["n0"] = pred2["n0"][0]
                pred["n0_stress"] = pred["n0_stress"][0]
                pred2["n0_stress"] = pred2["n0_stress"][0]
            else:
                data_c = deepcopy(data)
            ...
            if is_mesh:
                ...
                pred_2["n0_stress"] = pred_2["n0_stress"][0]
                pred2_2["n0_stress"] = pred2_2["n0_stress"][0]
            else:
                data_c = deepcopy(data)
