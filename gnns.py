...
from pytorch_net.util import to_np_array, init_args, to_cpu, Attr_Dict
from pytorch_net.net import fill_triangular, matrix_diag_transform
from utils_model import FCBlock
from utils import p, copy_data, sample_reward_beta, deepsnap_to_pyg, attrdict_to_pygdict, loss_op_core, parse_multi_step, to_tuple_shape, get_activation, seed_everything, edge_index_to_num, add_edge_normal_curvature, load_data
...
class processor_mean(MessagePassing):
    def __init__(
        ...
        super(processor_mean, self).__init__(aggr="mean")  # "mean" aggregation.
...
class processor_attention(nn.Module):
    def __init__(
        ...
        super(processor_attention, self).__init__()  
        self.edge_attr = edge_attr
        ...
        if self.edge_attr:
            in_features_edge = self.mid_channels     
        in_features_node = self.mid_channels
            
        self.node_query_layer = nn.Linear(self.in_channels, self.mid_channels,bias = False)
        self.node_key_layer = nn.Linear(self.in_channels, self.mid_channels,bias = False)
        self.node_value_layer = nn.Linear(self.in_channels, self.mid_channels,bias = False)

        self.edge_query_layer = nn.Linear(self.in_channels, self.mid_channels,bias = False)
        self.edge_key_layer = nn.Linear(self.in_channels, self.mid_channels,bias = False)
        self.edge_value_layer = nn.Linear(self.in_channels, self.mid_channels,bias = False)

        self.edge_encoder = FCBlock(in_features=in_features_edge, 
                                    out_features=self.out_channels,
                                    num_hidden_layers=2,
                                    hidden_features=self.mid_channels,
                                    outermost_linear=True,
                                    nonlinearity=act_name,
                                    layer_norm=layer_norm,
                                   )
        self.node_encoder = FCBlock(in_features=in_features_node,
                                    out_features=self.out_channels,
                                    num_hidden_layers=2,
                                    hidden_features=self.mid_channels,
                                    outermost_linear=True,
    ...
    def forward(self, graph):
        ...
        node_features_query = self.node_query_layer(graph.x)       
        node_features_key = self.node_key_layer(graph.x)
        node_features_value = self.node_value_layer(graph.x)
        ...
        # linear transform of edge_features
        edge_features_query = self.edge_query_layer(graph.edge_attr)       
        edge_features_key = self.edge_key_layer(graph.edge_attr)
        edge_features_value = self.edge_value_layer(graph.edge_attr)
        edge_features_query_head = edge_features_query.view(-1, self.attention_head, int(self.mid_channels / self.attention_head)).permute(1, 0, 2)
        edge_features_key_head = edge_features_key.view(-1, self.attention_head, int(self.mid_channels / self.attention_head)).permute(1, 0, 2) 
        edge_features_value_head = edge_features_value.view(-1, self.attention_head, int(self.mid_channels / self.attention_head)).permute(1, 0, 2) 
        ...
        alpha2 = torch.transpose(torch.sum(edge_features_query_head * node_features_key_head[:,receivers,:], dim = -1) / torch.sqrt(
            torch.tensor(self.mid_channels / self.attention_head, dtype=torch.float32)), 0, 1) 
        alpha3 = torch.transpose(torch.sum(edge_features_query_head * node_features_key_head[:,senders,:], dim = -1) / torch.sqrt(
            torch.tensor(self.mid_channels / self.attention_head, dtype=torch.float32)), 0, 1)   
        ...
        agg_edge_features = ((edge_attention[:,:,0:1] * edge_features_value_head).permute(1,2,0) 
                          + (edge_attention[:,:,1:2] * node_features_value_head[:,receivers,:]).permute(1,2,0) 
                          + (edge_attention[:,:,2:3] * node_features_value_head[:,senders,:]).permute(1,2,0)).permute(0,2,1)        
        edge_latent = agg_edge_features.reshape(-1, self.mid_channels)   
        graph.edge_attr = self.edge_encoder(edge_latent) + graph.edge_attr
        # end edge features aggregation ############################
        ...
        belta1 = torch.transpose(torch.sum(node_features_query_head * node_features_key_head, dim=-1) / torch.sqrt(
            torch.tensor(self.mid_channels / self.attention_head, dtype=torch.float32)), 0, 1)  
        belta2 = torch.transpose(torch.sum(node_features_query_head[:,receivers,:] * edge_features_key_head, dim=-1) / torch.sqrt(
            torch.tensor(self.mid_channels / self.attention_head, dtype=torch.float32)), 0, 1)
        ...
        agg_node_features = ((point2point_attention * node_features_value_head).permute(1,2,0) 
        ...
        node_latent = agg_node_features.reshape(-1, self.mid_channels) 
        graph.x = self.node_encoder(node_latent) + graph.x
        return graph  
...
def get_data_dropout(data, dropout_mode, exclude_idx=None, sample_idx=None,output_size=25):
    """
    ...
    if dropout_target in ["node", "uniform"]:
        ...
        if hasattr(data, "node_feature"):
            ...
            if hasattr(data, "edge_attr"):
                relative_mesh_pos = data_new.node_pos["n0"][data_new.edge_index[("n0", "0", "n0")][0]] - data_new.node_pos["n0"][data_new.edge_index[("n0", "0", "n0")][1]]
                relative_world_pos = data_new.node_feature["n0"][...,:output_size][data_new.edge_index[("n0", "0", "n0")][0]] - data_new.node_feature["n0"][...,:output_size][data_new.edge_index[("n0", "0", "n0")][1]]
                ...
                if data.edge_attr[("n0", "0", "n0")].shape[-1] == 1:
                    data_new["edge_attr"] = {("n0", "0", "n0"): relative_mesh_pos}
                elif data.edge_attr[("n0", "0", "n0")].shape[-1] == 52:
                    data_new["edge_attr"] = {("n0", "0", "n0"): torch.cat([edge_attr, edge_attr.abs()], -1)}
                else:
                    raise
        else:
            data_new = Data(
            ...
            if hasattr(data, "edge_attr"):
                relative_mesh_pos = data_new.x_pos[data_new.edge_index[0]] - data_new.x_pos[data_new.edge_index[1]]
                relative_world_pos = data_new.x[...,:output_size][data_new.edge_index[0]] - data_new.x[...,:output_size][data_new.edge_index[1]]
                edge_attr = torch.cat([relative_mesh_pos,relative_world_pos.reshape(relative_world_pos.size(0),-1)],-1) 
                if data.edge_attr.shape[-1] == 1:
                    data_new.edge_attr = relative_mesh_pos
                elif data.edge_attr.shape[-1] == 52:
                    data_new.edge_attr = torch.cat([edge_attr, edge_attr.abs()], -1)
                else:
                    raise
        ...
        if pooling_type in ["global_max_pool","global_mean_pool","global_mean_pool"]:
            if (processors[0]==None):
                ...
                for _ in range(num_steps):
                    if self.processor_aggr=="attention":
                        self.processors.append((processor_attention(latent_dim, 
                                                                    latent_dim, 
                                                                    layer_norm=layer_norm, 
                                                                    act_name=act_name, 
                                                                    edge_attr=edge_attr,
...
class GNNRemesher(nn.Module):
    ...
    def __init__(
        ...
        if diffMLP:
            ...
            self.processors = []
            for _ in range(num_steps):
                self.processors.append((processor_attention(latent_dim, latent_dim, layer_norm=layer_norm, act_name=act_name, edge_attr=edge_attr)))
                if batch_norm:
                    self.processors.append(torch.nn.BatchNorm1d(latent_dim))
                    self.processors.append(torch.nn.BatchNorm1d(latent_dim))
            self.processors = torch.nn.Sequential(*self.processors)
        else:
            self.processors = ((processor_attention(latent_dim, latent_dim, layer_norm=layer_norm, act_name=act_name, edge_attr=edge_attr)))
        ...
        if stress:
            self.decoder_node_stress = FCBlock(in_features = latent_dim,
                                        out_features= 1 + sizing_field_dim, #Von Mises
                                        num_hidden_layers=3,
                                        hidden_features=latent_dim,
                                        outermost_linear=True,
                                        nonlinearity=act_name,
    ...
    def decoder(self, graph):
        ...
        """
        node_latent = graph.x
        graph.x = self.decoder_node(node_latent)
        if self.stress:
            graph.stress =self.decoder_node_stress(node_latent) 
        if self.edge_threshold > 0:
    ...
    def forward_one_step(
        ...
        **kwargs
        ...
        if self.stress:
            graph_evolved.stress = graph_evolved.stress/10 #div 10 for 46, div 100 for 31
        # graph_evolved.x = graph_evolved.x #div 10 for 46, div 100 for 31
        ...
        if self.stress:
            graph_evolved.stress = graph_evolved.stress
            pred["n0_stress"] = graph_evolved.stress
        info = {}
        return pred, info
        ...
        **kwargs
        ...
        for k in range(1, max_pred_step+1):
            ...
            if hasattr(data, "edge_attr"):
                ...
                relative_world_pos = x_core[data.edge_index[0].to(data.x_pos.device)] - x_core[data.edge_index[1].to(data.x_pos.device)]
                edge_attr = torch.cat([relative_mesh_pos,relative_world_pos.reshape(relative_world_pos.size(0),-1)],-1)
                if isinstance(data.dataset,list):
                    dataset_str = data.dataset[0]
                elif isinstance(data.dataset,str):
    ...
    def get_loss(self, data, args, **kwargs):
        ...
        if args.data_dropout != "None" and self.training:
            if args.dataset.startswith("mppde1d"):
                if args.exclude_bdd:
                    ...
                    sample_idx = None
                data = get_data_dropout(data, dropout_mode=args.data_dropout, sample_idx=sample_idx,output_size = self.output_size)
        ...
        if len(dict(to_tuple_shape(data.original_shape))["n0"]) == 0:
            ...
            ylist = data.y_back
            if self.stress:
                ylist_stress = data.y_back_stress
        else:  
            t_start = time.time()
        ...
        if len(dict(to_tuple_shape(data.original_shape))["n0"]) == 0:
            ...
            for pred_idx, k in enumerate(multi_step_dict):
                loss_k = loss_op_core(
                    pred["n0"][pred_idx].flatten(start_dim=1),
                ...
                loss = loss + loss_k
                if self.stress:
                    loss_k_stress = loss_op_core(
                        pred["n0_stress"][pred_idx].flatten(start_dim=1),
                        ylist_stress[k-1].to(data.x.device),
                        loss_type=args.loss_type,
                        normalize_mode=args.latent_loss_normalize_mode,
                        **kwargs,
                    ) * multi_step_dict[k]
                    loss = 100 * loss + loss_k_stress
                self.info[f"loss_{k}"] = loss.item()
        else:
            for pred_idx, k in enumerate(multi_step_dict):
                loss_k = loss_op_core(
...
class GNNRemesherPolicy(GNNRemesher):
    ...
    def one_step_interpolation_forward(
        ...
        **kwargs
        ...
        # if use_remeshing or changing_mesh or mode=="test_remeshing_2":
            ...
            tarvers = shift_history[-1][:,:3].clone().detach()
            vers = shift_history[0][:,:3].clone().detach()
            ...
            mesh = self.pyg_to_dolfin_mesh_tetramesh(vers, faces)
            ...
            faces, weights = self.generate_3d_baryweight(tarvers.cpu().numpy(), vers.cpu().numpy(), mesh, bvhtree)
        ...
        outvec = data.x + data.history[-1][:,3:]
        next_feature = torch.cat([data.history[-1][:,:3].clone(), outvec], dim=-1)

        if self.stress:                             
            outvec_stress = torch.sign(data.stress[:, :1]) * (torch.exp(abs(data.stress[:, :1])) - 1)
            data.history_stress = (data.history_stress[-1], outvec_stress)
        ...
        if self.stress:
            pred["n0_stress"].append(data.stress[:, :1])
            info["final_gnnout_stress"] = outvec_stress.clone()
        if self.reward_dim > 0:
            pred["minus_reward"] = torch.stack(pred["minus_reward"])
    ...
    def compute_kinematics(self, data, index):
        # return 
        ybatch = ((data.reind_yfeatures[index][:,0] - data.yfeatures[index][:,0])/2)
        # onehot_list = data.onehot_list[index+1][:,0] #for handles
        handles_index = torch.where(data.onehot_list[index+1][:,0]==1)[0]
    ...
    def interpolation_hist_forward(
        ...
        **kwargs
        ...
        # pdb.set_trace()
        if self.stress:
            info = {"data": [], "data_x": [], "data_face": [], "y_tar": [],"data_stress": []}
        else:
        ...
        for k in range(1, max_pred_step+1):
            if is_non_remeshing:
                ...
                hist_wfeature =  data.history[0][:,self.output_size:]
                hist_wfeature_stress = data.history_stress[0]
            # add noise
            # pdb.set_trace()
            if self.noise_amp > 0:
            ...
            past_hist_wfeature = hist_wfeature.clone()
            past_hist_wfeature_stress = hist_wfeature_stress.clone()
            # Compute x feature for nodes by taking subtraction
            velocity = data.history[-1][:,3:] - hist_wfeature
            data = data.copy()
            data.x = velocity
            ...
            outvec = data.x[:,:3] + data.history[-1][:,3:] 
            # outvec = data.x[:,:3] 
            ...
            if self.stress: 
                pred["n0_stress"].append(data.stress[:,:1])  
                symlog_stress =    torch.sign(data.history_stress[-1]) * torch.log(torch.abs(data.history_stress[-1]) + 1) + data.stress[:,:1]                       
                outvec_stress = torch.sign(symlog_stress) * (torch.exp(torch.abs(symlog_stress)) - 1)  
                info["data_stress"].append(outvec_stress) 

            if is_non_remeshing:
                next_feature = torch.cat([data.history[-1][:, :3], outvec], dim=-1)
                if self.stress:
                    next_stress_feature = outvec_stress
            else:  
            ...
            data.history = (data.history[-1], next_feature)
            if self.stress:
                data.history_stress =  (data.history_stress[-1], next_stress_feature) 
    ...
    def pyg_to_dolfin_mesh_tetramesh(self, vers, faces):
        tmesh = self.dolfin.Mesh()
        editor = self.dolfin.MeshEditor()
        editor.open(tmesh, 'tetrahedron', 3, 3)

        editor.init_vertices(vers.shape[0])
        for i in range(vers.shape[0]):
            editor.add_vertex(i, vers[i,:].cpu().numpy())
        editor.init_cells(faces.shape[0])
        for f in range(faces.shape[0]):
            editor.add_cell(f, faces[f].cpu().numpy())
    ...
    def generate_3d_baryweight(self, tarvers,vers, mesh, bvh_tree):
        faces = []
        weights = []
        for query in tarvers:
            face = bvh_tree.compute_first_entity_collision(self.dolfin.Point(query))
    ...
    def _create_dual_graph_tetramesh(self, torchmesh):
        """ Created dual graph and generate associated information for splitting in Tetramesh. 
        ...
        for i, face in enumerate(torchmesh.xfaces.T.cpu().numpy()):
            n0 = tuple(sorted([face[0], face[1]]))
            n1 = tuple(sorted([face[0], face[2]]))
            n2 = tuple(sorted([face[0], face[3]]))
            n3 = tuple(sorted([face[1], face[2]]))
            n4 = tuple(sorted([face[1], face[3]]))
            n5 = tuple(sorted([face[2], face[3]]))

            try:
                dedge_neighbor_dict[n0].update({n1, n2, n3, n4})
            except KeyError:
                dedge_neighbor_dict[n0] = {n0}.union({n1, n2, n3, n4})

            try:
                dedge_neighbor_dict[n1].update({n0, n2, n3, n5})
            except KeyError:
                dedge_neighbor_dict[n1] = {n1}.union({n0, n2, n3, n5})

            try:
                dedge_neighbor_dict[n2].update({n0, n1, n4, n5})
            except KeyError:
                dedge_neighbor_dict[n2] = {n2}.union({n0, n1, n4, n5})

            try:
                dedge_neighbor_dict[n3].update({n0, n4, n1, n5})
            except KeyError:
                dedge_neighbor_dict[n3] = {n2}.union({n0, n4, n1, n5})

            try:
                dedge_neighbor_dict[n4].update({n0, n3, n2, n5})
            except KeyError:
                dedge_neighbor_dict[n4] = {n2}.union({n0, n3, n2, n5})

            try:
                dedge_neighbor_dict[n5].update({n1, n3, n2, n4})
            except KeyError:
                dedge_neighbor_dict[n5] = {n2}.union({n1, n3, n2, n4}) 
    ...
    def del_node_with_obtuse_triangles(self, deg4dnodes, node_face_dict, torchmesh):
        ...
        list_dnodes = torch.tensor(list_dnodes).to(torchmesh.x.device)[min_mask].tolist()
        ...
        dnodes = set(torch.tensor(list_dnodes).to(torchmesh.x.device)[min_wmask].tolist())
    ...
    def face_edge_split(self, sampnodes, outmesh):
        ...
        traw_enlist = torch.cat(
            [
                outmesh.xfaces[[0,1,2,3], :],
                outmesh.xfaces[[3,0,1,2], :],
                outmesh.xfaces[[2,3,0,1], :],
                outmesh.xfaces[[1,2,3,0], :],
                outmesh.xfaces[[0,2,3,1], :],
                outmesh.xfaces[[1,3,0,2], :],
            ], -1
        )
        ...
        raw_face_mask = torch.stack(
            [
                ...
                torch.isin(edge_index_to_num(outmesh.xfaces[[0,2], :], self.nmax), hash_samnodes),
                torch.isin(edge_index_to_num(outmesh.xfaces[[0,3], :], self.nmax), hash_samnodes),
                torch.isin(edge_index_to_num(outmesh.xfaces[[1,2], :], self.nmax), hash_samnodes),
                torch.isin(edge_index_to_num(outmesh.xfaces[[1,3], :], self.nmax), hash_samnodes),
                torch.isin(edge_index_to_num(outmesh.xfaces[[2,3], :], self.nmax), hash_samnodes),
            ], dim=0
        )
        ...
        new_vertices = torch.arange(outmesh.x.shape[0], outmesh.x.shape[0]+temp_sort.shape[0]).to(outmesh.x.device)
        scat_new_vertices = new_vertices[inverse_indices][sort_index]
        ...
        all_refined_faces = torch.cat(
            [generating_mesh_components[[0,1,3,4], :], generating_mesh_components[[1,2,3,4], :]], dim=-1
        )

        ## Create newly refined and added edges
        newly_generated_edges = torch.cat(
            [
                generating_mesh_components[[1,0], :], generating_mesh_components[[0,1], :],
                ...
                generating_mesh_components[[1,4], :], generating_mesh_components[[4,1], :],
    ...
    def split(self, split_edges, outmesh):
        ...
        if outmesh.xfaces.shape[0] == 0:
            ...
            if self.dataset.startswith("arcsim"):
            ...
            elif self.dataset.startswith("tetra"):
                # split_pts,split_pts_phys, split_pts_pos, split_pts_batch, split_pts_x_coords, remained_faces, all_refined_faces, remained_edge_index, new_edge_index = self.face_edge_split(split_edges, outmesh)
                split_pts,split_pts_phys, split_pts_pos, split_pts_batch, split_pts_x_coords, remained_faces, all_refined_faces, remained_edge_index, new_edge_index, split_onehot  = self.face_edge_split(split_edges, outmesh)
                outmesh.x_phy = torch.cat([outmesh.x_phy, split_pts_phys], 0)
                # print(outmesh.batch, split_pts_batch)
...
class GNNPolicyAgent_Sampling(GNNRemesherPolicy):
        ...
        **kwargs
        ...
        if self.processor_aggr=="max":
            ...
            if self.share_processor:
                ...
                for _ in range(num_steps):
                    ...
                    if self.batch_norm:
                        ...
                        self.value_processors.append(torch.nn.BatchNorm1d(latent_dim))
                self.value_processors = torch.nn.Sequential(*self.value_processors)       
        elif self.processor_aggr=="attention":
            self.processors = []
            for _ in range(num_steps):
                self.processors.append((processor_attention(latent_dim, 
                                                latent_dim, 
            ...
            if self.share_processor:
                ...
                self.value_processors = []
                for _ in range(num_steps):
                    self.value_processors.append((processor_attention(latent_dim, 
                                                    latent_dim, 
                                                    layer_norm=layer_norm, 
                                                    act_name=act_name, 
                                                    edge_attr=edge_attr,
                                                    attention_head= 8,
                                                    name = "value_processor_GT"
                                                    )))
                    if self.batch_norm:
                        self.value_processors.append(torch.nn.BatchNorm1d(latent_dim))
                        self.value_processors.append(torch.nn.BatchNorm1d(latent_dim))
        ...
        self.value_model = Value_Model(input_size=self.input_size,
                                        ...
                                        processor_aggr = self.processor_aggr,
                                      )
    ...
    def filter_with_dual_graph(self, torchmesh=None, sampled_edges=None):
        ...
        if self.dataset.startswith("m"):
            ...
            _, dedge_neighbor_dict = self._create_dual_graph_tetramesh(torchmesh)
        actual_sampled_edge = []
    ...
    def choose_split_edge_GNNtopK(self, outmesh,K_split=None, reward_beta=None,is_timing=0,is_eval_sample=True):
        ...
        if outmesh.xfaces.shape[0] == 0:
            ...
            if self.dataset.startswith("m"):
                ...
                receivers = outmesh.x_coords[tor_samnodes[:, 0], :3]
                senders = outmesh.x_coords[tor_samnodes[:, 1], :3]
                edge_size = torch.sqrt(((receivers - senders)**2).mean(dim=-1))
                mask = edge_size>self.min_edge_size*2
                splitable_edge = torch.where(mask==1)[0]
                tor_samnodes = tor_samnodes[splitable_edge]
    ...
    def remeshing_forward_GNN(self, torchmesh, use_pos=False,reward_beta=None,is_timing=False,interp_index=None,is_eval_sample=True,debug=False,data_gt=None,policy_input_feature="velocity",heuristic=False,**kwarg):
        ...
        if self.dataset.startswith("a"):
            if not torch.is_tensor(torchmesh.xface_list[0]):
                old_xfaces = torch.tensor(torchmesh.xface_list[0]).T
            else :
                old_xfaces = torchmesh.xface_list[0].T
        ...
        if self.dataset.startswith("m"):
            # outmesh.dataset = to_tuple_shape(outmesh.dataset)##冯波修改2024/04/07
            outmesh.dataset = outmesh.dataset
            #reset bdd nodes, all interpolate boundary node are removed
            outmesh.x[:,-1] = 1*(outmesh.x[:,-1]==1)
            outmesh.x_bdd = outmesh.x[:,-1:]
            outmesh.x =  outmesh.x[:,:-1]
            outmesh = update_edge_attr_1d(outmesh)
            outmesh.x = torch.cat([outmesh.x,outmesh.x_bdd],-1)
